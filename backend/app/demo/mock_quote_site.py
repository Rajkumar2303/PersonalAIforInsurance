...
def _marketing_html() -> str:
    """Generic intro/marketing-style page that contains a "$" but NO quote.

    Regression fixture (Issue #8.5 Smoke #1b): a dollar amount in body copy must
    NOT be enough to declare a quote. No quote-result heading, no premium label
    on an amount line.
    """
    return """<!doctype html><html><head><meta charset="utf-8"><title>Mock Marketing</title></head>
<body>
<h1>Welcome to Mock</h1>
<p>Save up to $500 a year on auto insurance with our bundle discounts.</p>
<button type="button" id="start-quote">Start my quote</button>
</body></html>"""


def _form_hint_html() -> str:
    """Normal initial form with instructional copy - NOT a validation error.

    Regression fixture (Issue #8.5 Smoke #1c): "Please select your province" is
    an instruction; a header "Log in" and a privacy/security footer must not
    count as an access barrier.
    """
    return """<!doctype html><html><head><meta charset="utf-8"><title>Mock Form</title></head>
<body>
<header><nav><a href="/login">Log in</a></nav></header>
<h1>Vehicle Information</h1>
<form method="post" action="/submit">
  <div><label for="province">Province</label>
    <select id="province" name="province"><option value="">Please select your province</option></select></div>
  ...
  <button type="submit" id="continue-form">Continue</button>
</form>
<footer>Privacy | Security | &copy; 2026 Protected. Terms of Use.</footer>
</body></html>"""


def _aria_invalid_html() -> str:
    """A control flagged aria-invalid=true with a visible error (real validation)."""
    return """<!doctype html><html><head><meta charset="utf-8"><title>Mock Invalid</title></head>
<body>
<h1>Postal Code</h1>
<form method="post" action="/submit">
  <div><label for="postal">Postal code</label>
    <input id="postal" name="postal_code" type="text" aria-invalid="true" aria-describedby="postal-error"></div>
  <p id="postal-error" class="field-error">Please enter a valid postal code</p>
  <button type="submit" id="continue-invalid">Continue</button>
</form>
</body></html>"""


def _role_alert_html() -> str:
    """A visible role=alert carrying validation-failure wording (real validation)."""
    return """<!doctype html><html><head><meta charset="utf-8"><title>Mock Alert</title></head>
<body>
<h1>Annual Kilometres</h1>
<div role="alert" id="annual-error">Please enter annual kilometres between 1,000 and 100,000</div>
<form method="post" action="/submit">
  <div><label for="annual-km">Approximate annual distance driven</label>
    <input id="annual-km" name="annual_km" type="number" required></div>
  <button type="submit" id="continue-alert">Continue</button>
</form>
</body></html>"""


def _recaptcha_badge_html() -> str:
    """Passive reCAPTCHA Enterprise badge (invisible/badge mode) - NOT a blocker.

    Regression fixture (Issue #8.5 Smoke #1d): a visible "Protected by reCAPTCHA"
    footer badge with NO challenge iframe/checkbox must be bot-protection
    PRESENCE only (access_control_detected = False).
    """
    return """<!doctype html><html><head><meta charset="utf-8"><title>Mock Badge</title></head>
<body>
<h1>Welcome</h1>
<div class="g-recaptcha" id="recaptcha-enterprise-container">Protected by reCAPTCHA<br>Privacy<br>Terms</div>
</body></html>"""


def _recaptcha_hidden_iframe_html() -> str:
    """Hidden reCAPTCHA Enterprise token iframe only - NOT a blocker."""
    return """<!doctype html><html><head><meta charset="utf-8"><title>Mock Hidden</title></head>
<body>
<h1>Welcome</h1>
<iframe src="https://www.recaptcha.net/recaptcha/enterprise/anchor?ar=1&k=test" style="display:none"></iframe>
</body></html>"""


def _recaptcha_challenge_html() -> str:
    """VISIBLE reCAPTCHA challenge widget (visible iframe inside g-recaptcha)."""
    return """<!doctype html><html><head><meta charset="utf-8"><title>Mock Challenge</title></head>
<body>
<h1>Security Check</h1>
<div class="g-recaptcha" id="recaptcha-widget">
  <iframe src="https://www.google.com/recaptcha/api2/anchor"></iframe>
</div>
</body></html>"""


def _hcaptcha_challenge_html() -> str:
    """VISIBLE hCaptcha challenge widget (visible iframe inside h-captcha)."""
    return """<!doctype html><html><head><meta charset="utf-8"><title>Mock hCaptcha</title></head>
<body>
<h1>Security Check</h1>
<div class="h-captcha" id="hcaptcha-widget">
  <iframe src="https://js.hcaptcha.com/captcha/v1/test"></iframe>
</div>
</body></html>"""


def _angular_pristine_html() -> str:
    """Pristine required Angular form with ng-invalid - NOT a validation error.

    Regression fixture (Issue #8.5 Smoke #1d): the form is ng-invalid because the
    required province is still empty, the aria-invalid input is HIDDEN, and
    "Please select your province" is instructional copy.
    """
    return """<!doctype html><html><head><meta charset="utf-8"><title>Mock Pristine</title></head>
<body>
<h1>Province</h1>
<form name="provinceForm" class="form-fluid sm-inline-form ng-pristine ng-invalid ng-invalid-required">
  <label for="province">Province</label>
  <div id="provinceButton" role="listbox">Select</div>
  <input type="hidden" id="province" name="province" aria-invalid="true"
         class="ng-pristine ng-untouched ng-empty ng-invalid ng-invalid-required">
  <p>Please select your province.</p>
  <button type="button" id="province-submit-btn">Continue</button>
</form>
</body></html>"""


def _touched_invalid_html() -> str:
    """Touched/dirty VISIBLE invalid field + visible error (real validation)."""
    return """<!doctype html><html><head><meta charset="utf-8"><title>Mock Touched</title></head>
<body>
<h1>Postal Code</h1>
<form name="postalForm" method="post" action="/submit">
  <label for="postal">Postal code</label>
  <input id="postal" name="postal_code" type="text" aria-invalid="true" aria-describedby="postal-error"
         class="ng-touched ng-dirty ng-invalid ng-invalid-required">
  <p id="postal-error" class="error">Please enter a valid postal code</p>
  <button type="submit" id="continue-touched">Continue</button>
</form>
</body></html>"""
...
<body>
<h1>Annual Kilometres</h1>
<form name="kmForm" class="ng-submitted ng-invalid">
  <label for="annual-km">Approximate annual distance driven</label>
...
_HTML_PAGES: dict[str, str] = {
    ...
    "/marketing": _marketing_html(),
    "/form-hint": _form_hint_html(),
    "/aria-invalid": _aria_invalid_html(),
    "/role-alert": _role_alert_html(),
    "/recaptcha-badge": _recaptcha_badge_html(),
    "/recaptcha-hidden-iframe": _recaptcha_hidden_iframe_html(),
    "/recaptcha-challenge": _recaptcha_challenge_html(),
...
class MockQuoteSite:
    ...
    Binds localhost only. ``port`` defaults to 0 (ephemeral) for tests; a
    fixed port may be supplied by the explicit dev/mock runtime so the data-
    driven route config can point at a stable start URL. Never a live
    destination: sandbox allowed_hosts defaults to localhost only and LIVE
    validation requires a verified route, which mock entries never are.
    """
